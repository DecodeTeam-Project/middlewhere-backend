...
WHERE some_column=some_value `
  ...
  projectBelongsToUser(projectId, userId) {
    ...
    .then((results) => {
      if (results.length === 1) {
        console.log("IT WAS TRUE MW 269");
        return true;
      }
  ...
  taskBelongsToUser(taskId, userId) { //userId
    return this.conn.query(
      `SELECT * FROM usersForTask
        WHERE usersForTask.taskId=? AND usersForTask.userId=?`,
        [taskId, userId]
    )
    .then((results) => {
      if (results.length === 1) {
  ...
  updateTaskComplete(taskId, completed) {
    console.log("MW, 425 " ,taskId, completed);
    return this.query(
      knex('tasks')
